...
public class MirrorSpectrumRenderer implements SpectrumRenderer {
    ...
    public void render(GuiGraphicsExtractor context, int screenWidth, int screenHeight,
        ...
        for (int i = 0; i < barCount; i++) {
            ...
            int barHeight = Math.max(1, (int) (level * maxHeight));

            int x = baseX + i * (barWidth + barGap);

            int topY = baseY - barHeight;
            context.fill(x, topY, x + barWidth, baseY, lerpColor(level));

            context.fill(x, baseY, x + barWidth, baseY + barHeight, lerpColor(level));
